...
import {
  ...
  Grid, Rating, Collapse, Alert, CircularProgress, Divider, Chip,
} from '@mui/material'
import {
...
const REFERRAL_OPTIONS = ['검색', 'SNS', '지인 소개', '우연히', '기타']
...
export default function GuestbookForm({ onSubmitted }) {
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 3 }}>
        <Typography variant="h3" sx={{ fontSize: '1.1rem', fontWeight: 700, mb: 2.5, color: 'var(--color-text-primary)' }}>
          ✍️ 방명록 남기기
        </Typography>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center', mb: 2 }}>
          <Box>
            <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: 'var(--color-text-secondary)' }}>
              이모지 선택
            </Typography>
              ...
              {EMOJI_LIST.map((e) => (
                <Box
                  ...
                  sx={{
                    ...
                    border: '2px solid',
                    borderColor: form.emoji === e ? 'var(--color-secondary)' : 'transparent',
                    backgroundColor: form.emoji === e ? 'rgba(45,212,191,0.12)' : 'transparent',
        ...
        {/* 선택 정보 토글 */}
        <Divider sx={{ my: 1.5, borderColor: 'var(--color-border-dark)' }} />
        <Button
          size="small"
        ...
        <Collapse in={expanded}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid size={12}>
              <Typography variant="caption" sx={{ display: 'block', mb: 1, color: 'var(--color-text-secondary)' }}>
                어떻게 알게 되셨나요?
              </Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.8, mb: form.referral_source === '기타' ? 1.5 : 0 }}>
                {REFERRAL_OPTIONS.map((opt) => (
                  <Chip
                    ...
                    sx={{
                      cursor: 'pointer',
                      fontWeight: form.referral_source === opt ? 700 : 400,
                      backgroundColor: form.referral_source === opt ? 'var(--color-secondary)' : 'transparent',
                      color: form.referral_source === opt ? 'var(--color-primary-dark)' : 'var(--color-text-secondary)',

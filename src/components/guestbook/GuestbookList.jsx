...
import { Box, Typography, Button, Skeleton, Card, CardContent, CircularProgress } from '@mui/material'
      ...
      <Box>
        {[1, 2, 3].map((i) => (
          <Card
            key={i}
            elevation={0}
            sx={{ border: '1.5px solid', borderColor: 'var(--color-border-dark)', borderRadius: 3, mb: 2, backgroundColor: 'var(--color-bg-card)' }}
          >
    ...
    return (
      <Box
        sx={{
          ...
          border: '2px dashed', borderColor: 'var(--color-border-gold)',
          borderRadius: 3, backgroundColor: 'rgba(45,212,191,0.03)',
        }}
      >
        <Typography variant="h2" sx={{ fontSize: '3rem', mb: 1 }}>📮</Typography>
        <Typography variant="h3" sx={{ fontSize: '1rem', color: 'var(--color-text-secondary)' }}>
          아직 방명록이 없습니다
        </Typography>
        <Typography variant="body2" sx={{ mt: 0.5, color: 'var(--color-text-secondary)' }}>
          첫 번째로 방명록을 남겨보세요! 🎉
        </Typography>
      </Box>
    )
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Button
          ...
          sx={{ color: 'var(--color-text-secondary)', fontSize: 12 }}
        >
          새로고침
        </Button>
      </Box>
